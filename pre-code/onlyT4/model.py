...
class SlowFast_Transformer(nn.Module):
    def __init__(self,
        ...
        for i in range(len(self.fast_dims)-1):
            self.inv_lateral_connections.append(
                nn.ConvTranspose2d(
                    in_channels=slow_dims[i],    # 예: Slow(128)
                    out_channels=fast_dims[i],   # 예: Fast(64)
                    kernel_size=(5, 1),
                    stride=(2, 1),
                    padding=(2, 0),
                    output_padding=(1, 0)        # [핵심] T=50 -> T=100으로 정확히 복원하기 위해 필수
                )
            )
    ...
    def forward(self, x_fast, x_slow):
        ...
        for i in range(len(self.fast_blocks)):
            # --- A. 양방향 융합 수행 (Bi-directional Lateral Connection) ---
            
            # [1] Fast -> Slow 융합 데이터 계산
            x_fast_permuted = x_fast.permute(0, 3, 1, 2).contiguous() # (N, C, T, J)
            fast_to_slow = self.lateral_connections[i](x_fast_permuted)
            fast_to_slow = fast_to_slow.permute(0, 2, 3, 1).contiguous() # (N, T, J, C)

            # [2] Slow -> Fast 융합 데이터 계산 (추가됨)
            x_slow_permuted = x_slow.permute(0, 3, 1, 2).contiguous() # (N, C, T, J)
            slow_to_fast = self.inv_lateral_connections[i](x_slow_permuted)
            slow_to_fast = slow_to_fast.permute(0, 2, 3, 1).contiguous() # (N, T, J, C)

            # [3] 교차 정보 주입 (동시 업데이트)
            # 주의: x_slow를 먼저 업데이트해버리면, 오염된 x_slow가 x_fast로 가게 됩니다.
            # 따라서 계산(1, 2번)을 다 끝낸 후 여기서 한꺼번에 더해줍니다.
            
            x_slow_combined = x_slow + fast_to_slow
            x_fast_combined = x_fast + slow_to_fast
            
            # --- B. GCN-Transformer 블록 실행 ---
            # 융합된 결과(combined)를 다음 블록에 넣습니다.
