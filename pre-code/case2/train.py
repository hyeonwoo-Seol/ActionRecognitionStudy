...
## >>>>>> python train.py --pretrained_path checkpoints/mae_pretrained_best.pth.tar --scheduler cosine_decay
# ## ----------------------------------------------------
...
def main():
    ...
    # >> 사전 학습된 모델 경로를 받는 인자이다.
    parser.add_argument(
        '--pretrained_path', type=str, default=None,
        help="Path to the MAE pre-trained checkpoint."
    )
    ...
    if args.pretrained_path:
        ...
        # 체크포인트 로드
        pretrained_checkpoint = torch.load(args.pretrained_path, map_location=device)
        pretrained_state_dict = pretrained_checkpoint['state_dict']
        
        # 현재 모델의 state_dict 가져오기
        model_state_dict = model.state_dict()

        # 사전학습 모델의 키에서 'reconstruction_head' 관련 가중치 제거
        pretrained_state_dict = {k: v for k, v in pretrained_state_dict.items() if "reconstruction_head" not in k}

        # 일치하는 키에 대해서만 가중치 업데이트
    ...
    # >> 차등 학습률 (Discriminative Learning Rates) 적용한다.
    if args.pretrained_path:
        print("Applying discriminative learning rates for fine-tuning.")
        # 파라미터를 그룹으로 나눔
        head_params = list(model.pose_encoder.parameters()) + \
                      list(model.attention_pool.parameters()) + \
                      list(model.fusion_layer.parameters()) + \
                      list(model.fc.parameters())
        
        backbone_params = [p for n, p in model.named_parameters() if not any(p is hp for hp in head_params)]

        # 각 그룹에 다른 학습률 설정
        optimizer = optim.AdamW([
            {'params': backbone_params, 'lr': config.LEARNING_RATE * 0.1}, # 백본은 1/10 학습률
            {'params': head_params, 'lr': config.LEARNING_RATE}             # 헤드는 원래 학습률
        ], weight_decay=config.ADAMW_WEIGHT_DECAY)
    else:
        # 처음부터 학습할 때는 모든 파라미터에 동일한 학습률 적용
        optimizer = optim.AdamW(
            model.parameters(),
