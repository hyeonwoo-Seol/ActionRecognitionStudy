...
def main():
    ...
    # <<-- 변경: HDF5 파일 여는 `with h5py.File(...)` 블록 전체가 아래의 단순 루프로 변경됨 -->>
    process_bar = tqdm(filenames, desc="[Processing and Saving files]")
    for filename in process_bar:
        ...
        if coords.shape[0] == 0:
            # <<-- HDF5는 빈 데이터를 저장하기 어려웠으나, .pt는 더미 데이터 저장이 용이 -->>
            ...
            # Pad or truncate
            processed_features = np.zeros((MAX_FRAMES, NUM_JOINTS, config.NUM_COORDS), dtype=np.float32)
            num_frames_to_copy = min(raw_features.shape[0], MAX_FRAMES)
            processed_features[:num_frames_to_copy] = raw_features[:num_frames_to_copy]

        action_id = int(filename[17:20])
        label = action_id - 1
        
        # <<-- 변경: HDF5에 순차적으로 쓰는 대신, 저장할 데이터를 딕셔너리로 구성 -->>
        data_to_save = {
        ...
        # <<-- 변경: 파일명을 .pt로 바꾸고 torch.save를 이용해 직접 저장 -->>
    ...
    print(f"\n모든 데이터 처리가 완료되었으며 '{TARGET_DATA_PATH}'에 저장되었습니다.")
