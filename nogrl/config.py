...
PROB = 0.7       # -> 데이터 증강 확률. 60% 확률로 증강을 '안'한다.
NUM_SUBJECTS = 40
...
ADAMW_WEIGHT_DECAY = 0.002  # AdamW weight decay , L2 정규화의 강도 설정
PATIENCE = 10              # 조기종료 변수
LABEL_SMOOTHING = 0.15     # Loss Function CrossEntropy의 label smoothing
DROPOUT = 0.45
ETA_MIN = 1e-6
T_0 = 100    # n에폭마다 학습률이 최대치로 초기화된다. 현재는 사용하지 않기 위해 100으로 설정했다.
T_MULT = 2   # 2로 설정된 경우, 15에폭 다음에 학습률 초기화는 2배인 30에폭이다. 현재는 사용하지 않는다.
ADVERSARIAL_ALPHA = 0.3    # 적대적 학습의 강도
