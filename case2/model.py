...
class GCNTransformerModel(nn.Module):
    def __init__(self,
        ...
        if self.mode == 'classification':
            ...
            # >> 최종 특징 벡터를 원래의 7차원 특징 벡터로 복원한다.
            self.reconstruction_head = nn.Linear(final_dim, num_coords)
        else:
            raise ValueError(f"Unknown mode: {mode}")

        

    def forward(self, motion_features, first_frame_coords = None, mask = None):
        ...
        if self.mode == 'classification':
            ...
            # --- 사전학습 모드 순전파 ---
            # x shape: (N, T, J, final_dim)
            final_dim = x.shape[-1]
            x_reconstructed = self.reconstruction_head(x) # (N, T, J, num_coords)
            
            # 마스킹된 부분의 예측 결과만 반환
            # mask shape: (N, T, J), boolean
            # 마스크를 (N, T, J, 1)로 확장하여 특징 차원과 브로드캐스팅
            masked_predictions = x_reconstructed[mask.unsqueeze(-1).expand_as(x_reconstructed)]
            # (num_masked_tokens, num_coords) 형태로 변환하여 반환
            return masked_predictions.view(-1, config.NUM_COORDS)
