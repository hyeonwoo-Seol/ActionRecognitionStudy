...
def resize_data_skateformer_style(data_numpy, target_frames=MAX_FRAMES):
    ...
    # (Real_T, M, V, C) -> (1, M*V*C, Real_T) 형태로 변환 (Batch, Channel, Length)
...
def calculate_and_save_stats():
    """통계(Mean, Std) 계산 및 분리 저장"""
    print("--- Calculating Stats (Using Interpolated Resizing) ---")
...
def main():
    ...
    print("--- Processing All Files (Using SkateFormer Style Resizing) ---")
    filenames = os.listdir(SOURCE_DATA_PATH)
    num_cores = cpu_count() - 1 if cpu_count() > 1 else 1
