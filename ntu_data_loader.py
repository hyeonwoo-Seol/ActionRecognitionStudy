...
class NTURGBDDataset(Dataset):
    ...
    def _load_data(self):
        # (이전과 동일)
        # ... (생략) ...
        print(f"Loading {self.split} data from '{self.data_path}'...")
    ...
    def __getitem__(self, index):
        ...
        features = saved_data['data'] # shape: (T, J, 6)
        ...
        if self.split == 'train':
            ...
            if np.random.rand() > 0.5:
                ...
                non_rotatable = features[..., :3]
                rotatable = features[..., 3:]
            ...
            if np.random.rand() > 0.5:
                noise = torch.randn_like(features) * 0.01 # 노이즈 수준(level)은 조절 가능
                features += noise
