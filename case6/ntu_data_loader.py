    ...
    def __getitem__(self, index):
        ...
        if self.split == 'train':
            ...
            if np.random.rand() > config.PROB:
                ...
                f_rel_ang = features[..., REL_ANG_CH] # (T, J, 2) [신규]
                f_inter_dist = features[..., INTER_DIST_CH] # (T, J, 2) [신규]
                ...
                # >> [수정됨] 모든 특징을 원래 순서대로 다시 결합 (13D)
                features = torch.cat([
                    f_dist, f_dir_rotated, f_acc_rotated, f_len, f_ang,
                    f_rel_ang, f_inter_dist
                ], dim=-1) # (T, J, 13)
