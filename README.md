...
그리고 첫 번째 프레임의 자세 정보를 별도로 추출하여 학습에 사용합니다. 이 정보는 MLP를 통과해 자세 벡터로 변환되고, 최종적으로 클래스를 분류하는 데 사용됩니다.
...
실험1에 칼만 필터를 추가해서 데이터 전처리 시 노이즈 증폭을 완화한다.

그리고 기본 Transformer 인코더 블록을 시공간 Transformer 인코더 블록으로 교체한다.

기존의 프레임별 joint의 이동 거리를 뼈 길이 정규화를 사용해서 구함으로써 서로 다른 크기의 사람의 행동을 일관성 있게 인식하도록 한다.
