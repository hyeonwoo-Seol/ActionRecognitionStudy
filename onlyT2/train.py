...
# >> python optimize.py 실행 중 Ctrl+c를 누르면 중단되고, 다시 python optimize.py를 실행하면
# >> 이어서 학습한다.

# >> 디버깅: python trian.py --trail-number 0 --lr 0.0003.... 등으로 단독 실행이 가능하다.
...
def run_trial(args, trail=None):
    try: # KeyboardInterrupt(Ctrl+C)를 잡기 위해 try 블록 시작
        ...
        # >> trial이 있으면 trial.number를, 없으면(단독 실행) args.trial_number를 사용한다.
        ...
        for epoch in range(start_epoch, MAX_EPOCHS_PER_TRIAL):
            ...
            if trail:
                # >> Pruning을 위한 현재의 epoch의 Validation Accuracy를 보고한다.
                trail.report(val_acc_action, epoch)

                # >> Pruning 여부를 확인한다.
                if trial.should_prune():
                    print(f"Trail {trial.number} pruned at epoch {epoch+1}.")
...
def main():
    ...
    # >> trail=None을 던잘하여 Pruning이 비활성화 될 수 있게 한다.
    best_acc = run_trail(args, trail=None)

    print(f"\n-- [Standard Run Finished: Trail {args.trail_number}]---")
    print(f"Best Validation Accuracy: {best_acc:.4f}")
    print(f"Checkpoints and history graph saved in: ")
    print(f"  {config.SAVE_DIR}/trail_{args.trail_number}/")
