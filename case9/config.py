...
# >> 모델 하이퍼파라미터
NUM_JOINTS = 50   # 관절 수
NUM_COORDS = 13   # 거리(1) + 방향(3) + 가속도(3) + 뼈길이(1) + 관절각도(1) + 몸통상대각도(2) + 비인접관절거리(2)
NUM_CLASSES = 60  # 행동 클래스 수 (NTU RGB+D 60)
PROB = 1.0 # 데이터 증강 확률. 70% 확률로 증강을 '안'한다.

# >> 학습 하이퍼파라미터
EPOCHS = 100             # 총 학습 에폭
LEARNING_RATE = 0.0001   # 
DEVICE = 'cuda' if torch.cuda.is_available() else 'cpu' # 학습 장치
WARMUP_EPOCHS = 4        # 학습 초기에 학습률을 서서히 증가시키는 웜업 에폭 수 
GRAD_CLIP_NORM = 1.0     # 그레이디언트 폭발을 막기 위한 클리핑 최대 L2 Norm 값 
ADAMW_WEIGHT_DECAY = 0.0 # AdamW weight decay , L2 정규화의 강도 설정
PATIENCE = 100 # 조기종료 변수
LABEL_SMOOTHING = 0.0 # Loss Function CrossEntropy의 label smoothing
DROPOUT = 0.0 # dropout
ETA_MIN = 1e-6
